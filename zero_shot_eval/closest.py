...
parser.add_argument("--trait_option", "-r", type=str, default='dorsal fin', choices=['dorsal fin','adipose fin','caudal fin','anal fin','pelvic fin','pectoral fin'])
parser.add_argument("--result_dir", "-o", type=str, default='/projects/ml4science/project_LMM/results/closest_detection', help="path to output")
parser.add_argument("--num_queries", "-n", type=int, default=5, help="number of images to query from dataset")


args = parser.parse_args()
args.result_dir = os.path.join(args.result_dir, args.trait_option)
...
##########################################################################################################################
from vlm_datasets.vqa_dataset import ClosestFinDataset
import jsonlines
import json 
...
closest_detection_dataset = ClosestFinDataset(
                                image_dir=image_dir,
...
args.num_queries = min(len(closest_detection_dataset), args.num_queries)

out_file_name = "{}/detection_{}_{}_num_{}.jsonl".format(args.result_dir, 
                                                            args.model, 
                                                            args.trait_option, 
                                                            args.num_queries)
...
for idx in tqdm(range(args.num_queries)):
    ...
    batch = closest_detection_dataset[idx]

    if os.path.exists(batch['image_path']) is False:
        print(f"{batch['image_path']} does not exist!")
        continue

    if args.trait_option not in list(batch["question_templates"].keys()):
        continue 
