...
parser.add_argument("--task_option", "-t", type=str, default='direct', choices=['direct', 'selection'], help="task option: 'direct', 'selection' ")
parser.add_argument("--size_option", "-s", type=str, default='largest', choices=['largest', 'smallest'])
parser.add_argument("--result_dir", "-o", type=str, default='/projects/ml4science/project_LMM/results/size_detection', help="path to output")
parser.add_argument("--num_queries", "-n", type=int, default=5, help="number of images to query from dataset")

args = parser.parse_args()
args.result_dir = os.path.join(args.result_dir, args.task_option, args.size_option)
...
##########################################################################################################################
from vlm_datasets.vqa_dataset import SizeDetectionDataset
import jsonlines
import json 
...
size_detection_dataset = SizeDetectionDataset(
                                image_dir=image_dir,
...
args.num_queries = min(len(size_detection_dataset), args.num_queries)
...
for idx in tqdm(range(args.num_queries)):
    ...
    batch = size_detection_dataset[idx]
    ...
    keys = f"{args.size_option}-{args.task_option}"

    target_output = batch['target_outputs'][keys]
    questions = batch['question_templates'][keys] 
    options = batch['option_templates'][keys] 
    answer_template = batch['answer_templates'][keys] 
    ...
    result["option-gt"] = batch['option_gt'][keys]
