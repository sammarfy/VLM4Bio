...
class DetectionGroundingDataset(BaseDataset):
    ...
    def get_question_template(self, bbox_trait):
        ...
        for trait in bbox_trait.keys():
            question_templates[trait] = f"{bbox_description} What is the Bounding Box coordinates of the {trait} in the fish shown in the image?"
    ...
    def get_options_template(self, bbox_trait):
        ...
        for trait in bbox_trait.keys():
            ...
            rand_options = list(np.random.choice(all_options, 3, replace=False))
    ...
    def __getitem__(self, idx):
        ...
        bbox_trait = self.get_trait_bbox_mapping(seg_map, present_traits, normalize_bbox=self.normalize_bbox) # returns dictionary of traits -> bounding boxes
...
class DetectionReferringDataset(BaseDataset):
    ...
    def get_question_template(self, bbox_trait):
        ...
        for trait in bbox_trait.keys():
            ...
            question_templates[trait] = f"{bbox_description} What is the trait of the fish that correspond to the bounding box region {bbox} in the image?"
        return question_templates
